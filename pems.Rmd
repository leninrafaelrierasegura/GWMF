...
```{r}
data_prd_list <- graph$get_mesh_locations(bru = TRUE, loc = "loc")
y_pred <- predict(rspde_model_stat, cmp_stat, rspde_fit_stat, newdata = data_prd_list, ~Intercept + field)
...
distance = seq(from = 0, to = 10, by = 1)
...
pointofinterest <- GROUPS[[5]][[indexofinterest]]
...
```{r, fig.cap = "MSE and negative Log-Score as functions of distance (in km) for the stationary (dotdash line, $\\boldsymbol{\\cdot-\\cdot}$) and non-stationary (solid line, $\\boldsymbol{-\\!\\!\\!-\\!\\!\\!-}$)  cases with $\\nu = 0.5$, $\\nu = 1.5$, and $\\nu$ estimated (est)."}
...
```{r}
save(combined_plot, file = here("data_files/pems_ggplot.RData"))
```
