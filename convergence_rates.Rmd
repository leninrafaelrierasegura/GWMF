...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(h_vector)) {
    h <- h_vector[i]
    m <- min(20, ceiling((min(2*alpha - 1/2,2) + 1/2)^2*log(h)^2/(4*pi^2*fract_alpha)))
...
transposed_df <- data.frame(t(data.frame(alpha = alpha_label, theoretical_rate = theoretical_rate, slope = slope_interval)))
...
colnames(df) <- c("h_vector", alpha_label)
...
transposed_df <- data.frame(t(data.frame(alpha = alpha_label, theoretical_rate = theoretical_rate, slope = slope_circle)))
...
transposed_df <- data.frame(t(data.frame(alpha = alpha_label, theoretical_rate = theoretical_rate, slope = slope_tadpole)))
...
```{r}
transposed_df <- data.frame(t(data.frame(alpha = alpha_label, 
...
```{r}
ggsave(here("data_files/conv_rates_new.png"), plot = last_plot, dpi = 300)
```
