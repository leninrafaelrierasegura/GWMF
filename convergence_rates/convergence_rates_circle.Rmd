---
title: "Convergence rates for the circle graph"
...
```{r, out.width= "50%", fig.cap = captioner("Circle graph with overkill mesh.")}
...
```{r, fig.dim = c(16, 20), fig.cap = captioner("Covariance error for the circle graph.")}
...
transposed_df <- data.frame(t(data.frame(alpha = alpha_label, theoretical_rate = theoretical_rate, slope1 = slope_circle_folded, slope2 = slope_circle_ok_mesh)))
...
```{r, fig.dim = c(12, 5), fig.cap = captioner("Covariance error for the circle graph, using overkill mesh and folded.")}
p + q
```
