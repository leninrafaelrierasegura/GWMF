...
Keep in mind the following relationships.
...
n_repl <- 40 # Number of replications
...
u_non_stat <- simulate(ns_op, nsim = n_repl)


list_fields <- list()
# Plot the simulated observations
for (i in 1:n_repl) {
  list_fields[[i]] <- logo_graph$plot_function(X = u_non_stat[,i], vertex_size = 0) + 
  ggtitle(paste("Field ", i)) + 
  theme_minimal() + 
  theme(text = element_text(family = "Palatino"))
}
do.call(grid.arrange, c(list_fields, ncol = 5))
```
...
for (k in 1:n_repl) {
  ...
  # Simulate observations
  Y <- c(Y, 1 - 3*x2 + as.vector(A %*% u_non_stat[, k] + sigma.e * rnorm(n.obs)))
  EDGE_NUMBER <- c(EDGE_NUMBER, obs.loc[,1])
  DISTANCE_ON_EDGE <- c(DISTANCE_ON_EDGE, obs.loc[,2])
  X2 <- c(X2, x2)
  REPL <- c(REPL, rep(k, n.obs))
  }
...
logo_graph$add_observations(data = df_data, group = "day", normalized = TRUE, clear_obs = TRUE)
...
# Plot the simulated observations
for (i in 1:n_repl) {
  list_plots[[i]] <- logo_graph$plot(data = "y", group = i, vertex_size = 0) + 
    ggtitle(paste("Replicate ", i)) + 
    theme_minimal() + 
    theme(text = element_text(family = "Palatino"))
}
do.call(grid.arrange, c(list_plots, ncol = 5))
```
...
for (i in 1:n_repl) {
  distmatrixlist[[i]] <- logo_graph$compute_geodist_PtE(PtE = aux %>% 
